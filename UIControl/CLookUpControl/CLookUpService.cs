...
{
    [ToolboxItem(true)]
    public class CLookUpService : CSmartLookUpEditBase
    {
        protected override void OnCreateControl()
        {
            ...
            if (Properties.Columns.Count == 0)
            {
				...
				{
					new LookUpColumnInfo("ServiceCode", "服务编号"),
					new LookUpColumnInfo("ServiceName", "服务名称"),
        ...
        public List<Service> ListIsValid { get; set; }
        ...
        public void BindList(bool getFromCache = true)
        {
            if (getFromCache)
            {
                ListIsValid = (List<Service>)ClientCache.GetAuto("Service", () => new ServiceDao().GetList());
            }
            else
            {
                ListIsValid = (List<Service>)ClientCache.GetUpdate("Service", () => new ServiceDao().GetList());
            }
            Properties.DataSource = ListIsValid;
            Properties.DisplayMember = "ServiceName";
            Properties.ValueMember = "ServiceID";
