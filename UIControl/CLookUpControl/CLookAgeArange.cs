...
{
    [ToolboxItem(true)]
    public partial class CLookAgeArange : CSmartLookUpEditBase
    {
        ...
        protected override void OnCreateControl()
        {
            ...
            if (Properties.Columns.Count == 0)
            {
				...
				//	new LookUpColumnInfo("SchoolCode", "学校编号"),
                    new LookUpColumnInfo("AgeRange", "年龄段"),
        ...
        public List<Agerange> ListIsValid { get; set; }
        public Agerange Currentschool { get; set; }
        ...
        {
            //FormAgerange form = new FormAgerange();
            //form.FormClosed += (s, e) =>
            //{
            //    BindList(false);
            //};
        ...
        public void BindList(bool getFromCache = true)
        {
            if (getFromCache)
            {
                ListIsValid = (List<Agerange>)ClientCache.GetAuto("AgeRange", () => new AgerangeDao().GetList());
            }
            else
            {
                ListIsValid = (List<Agerange>)ClientCache.GetUpdate("AgeRange", () => new AgerangeDao().GetList());
            }
            Properties.DataSource = ListIsValid;
            Properties.DisplayMember = "AgeRange";
            Properties.ValueMember = "AgeRangeID";
        ...
        private void cLookUp_EditValueChanged(object sender, EventArgs e)
        {
            ...
            Currentschool = EditValue == null
                   ? null
                   : ListIsValid.Find(model => model.AgeRangeID == Convert.ToInt32(EditValue));
        }
