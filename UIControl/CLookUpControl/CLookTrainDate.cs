...
{
    [ToolboxItem(true)]
    public class CLookTrainDate : CSmartLookUpEditBase
    {
        protected override void OnCreateControl()
        {
            ...
            if (Properties.Columns.Count == 0)
            {
				...
				{
					new LookUpColumnInfo("TrainDateCode", "培训时间编号"),
					new LookUpColumnInfo("TrainDateName", "培训时间"),
        ...
        public List<Traindate> ListIsValid { get; set; }
        public Traindate _Current { get; set; }
        ...
        public void BindList(bool getFromCache = true)
        {
            if (getFromCache)
            {
                ListIsValid = (List<Traindate>)ClientCache.GetAuto("Traindate", () => new TraindateDao().GetList());
            }
            else
            {
                ListIsValid = (List<Traindate>)ClientCache.GetUpdate("Traindate", () => new TraindateDao().GetList());
            }
            Properties.DataSource = ListIsValid;
            Properties.DisplayMember = "TrainDateName";
            Properties.ValueMember = "TrainDateID";
