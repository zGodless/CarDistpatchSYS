...
{
    [ToolboxItem(true)]
    public class CLookUpTactic : CSmartLookUpEditBase
    {
        protected override void OnCreateControl()
        {
            ...
            if (Properties.Columns.Count == 0)
            {
				...
				{
					new LookUpColumnInfo("TacticName", "策略名称"),
        ...
        public List<Tactic> ListIsValid { get; set; }
        public Tactic _Current { get; set; }
        ...
        public void BindList(bool getFromCache = true)
        {
            if (getFromCache)
            {
                ListIsValid = (List<Tactic>)ClientCache.GetAuto("Tactic", () => new TacticDao().GetList());
            }
            else
            {
                ListIsValid = (List<Tactic>)ClientCache.GetUpdate("Tactic", () => new TacticDao().GetList());
            }
            Properties.DataSource = ListIsValid;
            Properties.DisplayMember = "TacticName";
            Properties.ValueMember = "TacticID";
        ...
        private void cLookUp_EditValueChanged(object sender, EventArgs e)
        {
            ...
            _Current = EditValue == null
                   ? null
                   : ListIsValid.Find(model => model.TacticID == Convert.ToInt32(EditValue));
        }
