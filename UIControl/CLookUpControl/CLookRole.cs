...
{
    [ToolboxItem(true)]
    public class CLookRole : CSmartLookUpEditBase
    {
        protected override void OnCreateControl()
        {
            ...
            if (Properties.Columns.Count == 0)
            {
                ...
                {
                    new LookUpColumnInfo("RoleName", "角色名称")
                });
            }
        ...
        public List<Role> ListIsValid { get; set; }
        public Role Currentduty { get; set; }
        ...
        public void BindList(bool getFromCache = true)
        {
            if (getFromCache)
            {
                ListIsValid = (List<Role>)ClientCache.GetAuto("Role", () => new RoleDao().GetList());
            }
            else
            {
                ListIsValid = (List<Role>)ClientCache.GetUpdate("Role", () => new RoleDao().GetList());
            }
            Properties.DataSource = ListIsValid;
            Properties.DisplayMember = "RoleName";
            Properties.ValueMember = "RoleID";
        ...
        private void cLookUp_EditValueChanged(object sender, EventArgs e)
        {
            ...
            Currentduty = EditValue == null
                   ? null
                   : ListIsValid.Find(model => model.RoleID == Convert.ToInt32(EditValue));
        }
