	...
	static class MsgBox
	{
		...
		public static void ShowError(Exception e, string title = null)
		{
			try
			{
				...
				if (msg.Contains("ERROR-CC"))
				{
					msg = msg.Contains("目标数组的长度不够")
						? "创建或访问本地缓存失败！\r\n\r\n由于网络延时，缓存下载不完整！\r\n请清除本地缓存，重新登录!"
						: "创建或访问本地缓存失败!请尝试:\r\n\r\n1）、检查安装目录访问权限；\r\n2）、清除本地缓存，重新登录";
				}
				else if (msg.Contains("检索COM类工厂中CLSID为"))
				{
					msg = "组件未注册!";
				}
		...
		public static DialogResult ShowYesNoCancel(string txt, string title = null)
		{
			try
			{
				...
				var m_form = new XtraMessageBoxForm();
				var xe = new XtraMessageBoxArgs(null, txt.Replace("<br/>", "\r\n"), title,
					new[] {DialogResult.Yes, DialogResult.No, DialogResult.Cancel}, Resources.WhatIcon, 0);
		...
		public static DialogResult ShowYesNo(string txt, int defaultButton = 0, string title = null)
		{
			try
			{
				...
				var xe = new XtraMessageBoxArgs(null, txt.Replace("<br/>", "\r\n"), title, new[] {DialogResult.Yes, DialogResult.No},
					Resources.QuestIcon, defaultButton);
				return m_form.ShowMessageBoxDialog(xe);
			}
			catch
			{
		...
		public static DialogResult ShowOKCancel(string txt, int defaultButton = 0, string title = null)
		{
			try
			{
				...
				var m_form = new XtraMessageBoxForm();
				var xe = new XtraMessageBoxArgs(null, txt.Replace("<br/>", "\r\n"), title,
					new[] {DialogResult.OK, DialogResult.Cancel}, Resources.QuestIcon, defaultButton);
				return m_form.ShowMessageBoxDialog(xe);
			}
			catch
			{
