...
namespace QuickFrame.DbConnector.DbDriver
{
	class DbPostgreSql : DbBase
	{
		...
		public DbPostgreSql(string binPath)
			: base(binPath)
		{
			DatabaseType = "PostgreSql";
		}
		...
		public override int ExecuteSql(string strSql, string content)
		{
			...
			{
				strSql = GetSql(strSql);
				var cmd = new NpgsqlCommand(strSql, connection) {CommandTimeout = Timeout};
				var myParameter = new NpgsqlParameter("@content", NpgsqlDbType.Text) {Value = content};
		...
		public override object ExecuteSqlGet(string strSql, string content)
		{
			using (var connection = new NpgsqlConnection(ConStr))
			{
				...
				cmd.CommandTimeout = Timeout;
				var myParameter = new NpgsqlParameter("@content", NpgsqlDbType.Text);
		...
		public override int ExecuteSqlInsertImg(string strSql, byte[] fs)
		{
			using (var connection = new NpgsqlConnection(ConStr))
			{
				...
				var cmd = new NpgsqlCommand(strSql, connection);
				cmd.CommandTimeout = Timeout;
				var myParameter = new NpgsqlParameter("@fs", NpgsqlDbType.Bytea);
		...
		public override DataSet Query(string strSql)
		{
			using (var connection = new NpgsqlConnection(ConStr))
			{
				...
				connection.Open();
				strSql = GetSql(strSql);
				var command = new NpgsqlDataAdapter(strSql, connection);
				command.Fill(ds, "ds");
				return ds;
			}
		}
		...
		public override int ExecuteSql(string strSql, params QfParameter[] cmdParms)
		{
			using (var connection = new NpgsqlConnection(ConStr))
			{
				...
				var rows = cmd.ExecuteNonQuery();
				BindOutputParams(cmd.Parameters.Cast<NpgsqlParameter>(), cmdParms);
				return rows;
			}
		}
		...
		///     执行多条SQL语句，实现数据库事务。
		/// </summary>
		/// <param name="strSqlList">SQL语句的哈希表（key为sql语句，value是该语句的NpgsqlParameter[]）</param>
		...
		public override void ExecuteSqlTranWithIndentity(List<QfCommandInfo> strSqlList)
		{
			using (var conn = new NpgsqlConnection(ConStr))
			{
				...
				using (var trans = conn.BeginTransaction())
				{
					var cmd = new NpgsqlCommand {CommandTimeout = Timeout};
					try
					{
						var indentity = 0;
						//循环
		...
		public override DataSet RunProcedure(string storedProcName, Dictionary<string, QfParameter> parameters)
		{
			using (var connection = new NpgsqlConnection(ConStr))
			{
				...
				connection.Open();
				var sqlDa = new NpgsqlDataAdapter {SelectCommand = (NpgsqlCommand)BuildQueryCommand(connection, storedProcName, parameters)};
				sqlDa.SelectCommand.CommandTimeout = Timeout;
				sqlDa.Fill(dataSet);
				connection.Close();
				BindOutputParams(sqlDa.SelectCommand.Parameters.Cast<NpgsqlParameter>(), parameters);
				return dataSet;
			}
		}
		...
		public override int RunProcedure(string storedProcName, QfParameter[] parameters, out int rowsAffected)
		{
			using (var connection = new NpgsqlConnection(ConStr))
			{
				...
				BindOutputParams(command.Parameters.Cast<NpgsqlParameter>(), parameters);
				return result;
			}
		}
		...
		private NpgsqlCommand BuildIntCommand(NpgsqlConnection connection, string storedProcName,
			QfParameter[] parameters)
		{
			var command = (NpgsqlCommand)BuildQueryCommand(connection, storedProcName, parameters);
			var param = new NpgsqlParameter("ReturnValue", NpgsqlDbType.Integer, 4);
