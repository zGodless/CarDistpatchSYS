...
namespace QuickFrame.DbConnector.DbDriver
{
	class DbSqlServer : DbBase
	{
		...
		public DbSqlServer(string binPath)
			: base(binPath)
		{
			DatabaseType = "SqlServer";
		}
		...
		public override bool ColumnExists(string tableName, string columnName)
		{
			...
			{
				var strSql = @"select count(1) from syscolumns where [id]=object_id('" + tableName + "') and [name]='" + columnName + "'";
		...
		public override int ExecuteSql(string strSql, string content)
		{
			...
			{
				strSql = GetSql(strSql);
				var cmd = new SqlCommand(strSql, connection) {CommandTimeout = Timeout};
				var myParameter = new SqlParameter("@content", SqlDbType.Text) {Value = content};
		...
		public override object ExecuteSqlGet(string strSql, string content)
		{
			using (var connection = new SqlConnection(ConStr))
			{
				...
				cmd.CommandTimeout = Timeout;
				var myParameter = new SqlParameter("@content", SqlDbType.Text);
		...
		public override int ExecuteSqlInsertImg(string strSql, byte[] fs)
		{
			using (var connection = new SqlConnection(ConStr))
			{
				...
				var cmd = new SqlCommand(strSql, connection);
				cmd.CommandTimeout = Timeout;
				var myParameter = new SqlParameter("@fs", SqlDbType.VarBinary);
		...
		public override int ExecuteSql(string strSql, params QfParameter[] cmdParms)
		{
			using (var connection = new SqlConnection(ConStr))
			{
				...
				var rows = cmd.ExecuteNonQuery();
				BindOutputParams(cmd.Parameters.Cast<SqlParameter>(), cmdParms);
				return rows;
			}
		}
		...
		public override DataSet RunProcedure(string storedProcName, Dictionary<string, QfParameter> parameters)
		{
			using (var connection = new SqlConnection(ConStr))
			{
				...
				connection.Open();
				var sqlDa = new SqlDataAdapter {SelectCommand = (SqlCommand)BuildQueryCommand(connection, storedProcName, parameters)};
				sqlDa.SelectCommand.CommandTimeout = Timeout;
				sqlDa.Fill(dataSet);
				connection.Close();
				BindOutputParams(sqlDa.SelectCommand.Parameters.Cast<SqlParameter>(), parameters);
				return dataSet;
			}
		}
		...
		public override int RunProcedure(string storedProcName, QfParameter[] parameters, out int rowsAffected)
		{
			using (var connection = new SqlConnection(ConStr))
			{
				...
				BindOutputParams(command.Parameters.Cast<SqlParameter>(), parameters);
				return result;
			}
		}
		...
		private SqlCommand BuildIntCommand(SqlConnection connection, string storedProcName,
			QfParameter[] parameters)
		{
			var command = (SqlCommand)BuildQueryCommand(connection, storedProcName, parameters);
